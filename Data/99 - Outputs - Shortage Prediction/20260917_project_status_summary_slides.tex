...
\begin{tabular}{lcc}
\toprule
Config (text only, full sample) & Old outcome & New outcome (relative) \\
\midrule
Logistic Regression & 0.483 (n.s.) & \highlight{0.589, $p{=}0.048$} \\
Random Forest        & 0.532 (n.s.) & \highlight{0.593, $p{=}0.041$} \\
\midrule
OAI/VAI flag alone   & 0.537 (n.s.) & \highlight{0.457 -- worse than random} \\
\bottomrule
\end{tabular}
}
\vspace{10pt}
New outcome: did this facility's own AE count rise after inspection vs.\ its own
pre-inspection window. Two things changed: text now predicts across the
\highlight{full sample}, not a small subgroup; FDA's own classification alone
predicts \highlight{nothing}.
\end{frame}
...
% ─────────────────────────────────────────────────────────────────────────────
\begin{frame}{Results Across All Three Models}
{\small
\begin{tabular}{lccc}
\toprule
Model & Baseline AUC & With-text AUC & Note \\
\midrule
Recall (m14)   & 0.26--0.34 & too few events & baseline below random \\
AE (m17)       & 0.57--0.60 & 0.43--0.49 & baseline improved after fix; with-text did not \\
Shortage (m19) & \highlight{0.54--0.61} & 0.49--0.50 & baseline \highlight{significant} \\
\bottomrule
\end{tabular}
}
\vspace{10pt}
Shortage baseline: $p{=}0.003$ (LR), $p{=}0.011$ (RF) -- \highlight{the strongest,
cleanest result in either pipeline right now, and it uses no text at all.}
\end{frame}
...
\begin{itemize}
  \setlength{\itemsep}{10pt}
  \item Plain inspection history (OAI/VAI counts, inspection frequency,
        warning letters) plus structural features (parenteral status, supply
        concentration) significantly predicts shortage exposure
  \item Only visible once the panel stopped being restricted to text-covered
        facilities -- every earlier version of this model missed it
  \item Does not confirm the text-specific VAI hypothesis on its own; it is a
        separate, additive finding about not discarding facilities without text
  \item Not yet stress-tested the way the AE story has been (repeated
        cohorts, multiple reruns) -- next step, not a finished result
\end{itemize}
\end{frame}
