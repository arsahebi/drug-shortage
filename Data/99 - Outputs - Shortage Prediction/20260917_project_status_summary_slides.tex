...
\vspace{10pt}
\begin{alertblock}{Why adding text hurts here}
\small The with-text subset is small (148--177 rows). Adding 12 text
features on top of 6 structural/inspection ones, on that few rows, spreads
too little data across too many features -- the model overfits noise instead
of learning signal. The baseline (6 features, 1{,}100+ rows) has room to
learn; the with-text model does not. This is a sample-size problem, not
evidence that text is uninformative.
\end{alertblock}
\vspace{6pt}
...
\begin{itemize}
  ...
  \item \highlight{Mostly supply concentration}, not quality per se:
        n\_feis\_drug (how many other facilities make the same drug) alone
        is 62\% of the signal -- sole-source facilities are the risk
  ...
  \item Not yet stress-tested the way the AE story has been -- next step, not
        a finished result
\end{itemize}
\end{frame}
