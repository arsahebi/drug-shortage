...
\begin{itemize}
  \setlength{\itemsep}{10pt}
  \item \textbf{The shortage test has now been run (m19).} Working hypothesis: OAI forces
        remediation and heads off a shortage; VAI does not force the same response, so an
        undetected quality problem at a VAI facility could keep degrading into a shortage. Direct
        test (text-only, VAI-only, shortage as outcome): inconclusive, AUC 0.51--0.59 on only 28
        events, too sparse to read either way.
  \item \textbf{An unrelated, more promising finding from the same model.} A plain inspection +
        ...
        exposure, AUC up to 0.61 -- evidence for the broader claim (quality/regulatory signals
        predict shortage) even though it doesn't confirm the text-specific VAI hypothesis above.
        Not yet significance-tested; see RESULTS.docx.
  \item \textbf{More data, not a different model.} The VAI-only cohort is still small (55
        facilities) because 483-text coverage is still short for many facilities. Revisit as
        coverage grows.
  \item \textbf{Align the OAI/VAI source.} This analysis still relies mainly on the
        Redica-derived classification (last refreshed 2026-07-15); m19 already switched to the
        fresher FDA Inspection Details file directly. Matching that here is the next concrete step.
\end{itemize}
\end{frame}
