...
Recalls are a cleaner prediction target than shortages for this FEI-level panel:
  ...
  - More events than shortages across the Redica-483-text-covered FEI universe
...
Two models, checked and run independently (2026-09-16):
  - Baseline: inspection + structural features only, on every FEI with Redica
    inspection history (not restricted to text coverage). A facility's
    inspection record is real signal on its own and shouldn't be discarded
    just because it lacks 483 text yet.
  ...
  Each model is skipped independently if it doesn't clear the minimum-events
  threshold; one running does not require the other to.
...
Outputs:
  outputs/models/metrics_recall_fei_baseline.csv (inspection+structural, full universe)
  outputs/models/metrics_recall_fei.csv          (with-text, text-covered subset -- if modeled)
  ...
  outputs/figures/text_feature_lift_recall_fei.png      (ablation Fig D -- if both models ran)
...
def build_panel() -> pd.DataFrame:
    ...
    # (9 more recall events across 27 FEIs, as of 2026-09-16). Text features are
    # still never zero-filled (see _prep): a facility-year with no as-of-year
...
def _write_panel_summary(panel: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"- **Rows with a valid outcome (baseline model population):** {len(modeled):,} "
          f"({modeled['fei'].nunique()} FEIs, {int(modeled['y_recall_next'].sum())} events)",
        f"- **Rows with an as-of-year text snapshot (with-text model population, no "
          ...
          f"{int(with_snapshot['y_recall_next'].sum())} events)",
        f"- **Recall events (y=1), full outcome set:** {int(modeled['y_recall_next'].sum())} "
        ...
        "restricted to the as-of-year-snapshot population. Each is skipped on its own if "
        "it doesn't clear the minimum-events threshold -- one running does not require the "
        "other to.",
        "",
        "## Feature coverage",
...
def main():
    ...
    # Checked and modeled independently of the with-text model below -- a
    # facility's inspection record shouldn't need 483 text to be usable.
    ...
    # naturally restricted to the text-covered subset -- the fair, apples-to-
    # apples population for the with/without-text ablation below.
    ...
    ablation_rows = []
    fig_source = None  # (label, X, y, groups, df, preds_l2, met_l2, preds_rf, met_rf)
    ...
    if baseline_ok:
        ...
        ]).to_csv(OUT_MODELS / "metrics_recall_fei_baseline.csv", index=False)
        ablation_rows.append({"label": "Without text\n(full universe)", "auc": met_l2_b["auc"], "model": "L2"})
        fig_source = ("baseline", X_insp, y_insp, g_insp, df_insp, preds_l2_b, met_l2_b, preds_rf_b, met_rf_b)
    else:
    ...
    if text_ok:
        ...
        fig_source = ("with-text", X_all, y_all, g_all, df_all, preds_l2_t, met_l2_t, preds_rf_t, met_rf_t)
    else:
    ...
    if fig_source is not None:
        label, X_fig, y_fig, g_fig, df_fig, preds_l2_f, met_l2_f, preds_rf_f, met_rf_f = fig_source
        ...
        _fig_risk_ranking(df_fig, rf_full, list(X_fig.columns), facility_names)
