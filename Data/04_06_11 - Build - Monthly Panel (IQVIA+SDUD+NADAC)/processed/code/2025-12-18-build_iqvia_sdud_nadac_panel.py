...
def load_nadac_monthly(nadac_dir: str, pricing_units: Optional[Set[str]] = None) -> pd.DataFrame:
    ...
    - nadac_price: mean of all weekly NADAC prices in that month
    - n_obs: count of weekly records in that month (sum across weeks)
    ...
    for f in files:
        ...
        # Deduplicate weekly records in case your downloaded files overlap
    ...
    nadac_m = (
        all_.groupby(["ndc11", "date"], as_index=False)
            .agg(
                nadac_price=("nadac_price", "mean"),     # avg of weekly prices in month
...
def build_panel(
    ...
    nadac = load_nadac_monthly(str(nadac_dir), pricing_units=pricing_units)
