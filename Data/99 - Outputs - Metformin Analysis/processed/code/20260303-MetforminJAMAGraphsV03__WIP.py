...
DF_FILE = BASE_DIR / "Q&As1234_v10_v4.xlsx"
...
# -------------------- NDC NORMALIZATION --------------------
# v10_v4 already provides ndc11 (11-digit, zero-padded) and NDC_542 (5-4-2 hyphenated).
# No format conversion is needed — just validate and rename.

def normalize_ndc_columns(d: pd.DataFrame) -> pd.DataFrame:
    """
    For v10_v4: rename ndc11 -> NDC11 and validate it is exactly 11 digits.
    NDC_542 is already present in the correct 5-4-2 hyphenated format.
    """
    out = d.copy()
    out = out.rename(columns={"ndc11": "NDC11"})
    out["NDC11"] = (
        out["NDC11"]
        .astype(str)
...
def build_ndc_year_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Steps:
      1) Normalize NDC columns (rename ndc11 -> NDC11, validate length).
      2) Apply measurement rules:
           - NDMA in 2024 -> NaN (not measured that year)
           - Difference Factor only measured in 2024; force NaN for 2020/2022
           - Do NOT convert NaN to 0 for Difference Factor
      3) Compute PriorScore per (NDC11, Year):
           - TestDate = Dec 31 of that Year
           - Most recent Event Start Date <= TestDate
           - Use Event Score if present, else Score
           - Snap to {0.0, 1.5, 3.5}
      4) Aggregate all metrics to one row per (NDC11, Year).
    """
    d = df.copy()
    ...
    # --- TestDate proxy: Dec 31 of each test year ---
    ...
    d.loc[d["Year"] == 2024, "NDMA"] = np.nan  # NDMA not measured in 2024

    d["Dissolution"] = pd.to_numeric(d.get(DISS_COL), errors="coerce")
    d.loc[d["Year"].isin([2020, 2022]), "Dissolution"] = np.nan  # only measured in 2024
    # IMPORTANT: do NOT fill NaN with 0

    # --- inspection score per row ---
    ...
    def prior_score_for_group(g: pd.DataFrame):
        """Most recent inspection at or before TestDate (year-end)."""
        ...
        return (last["ScoreUsed"] if pd.notna(last["ScoreUsed"]) else np.nan,
                last["Event Start Date"])

    # --- aggregate to one row per (NDC11, Year) ---
    agg = (
        d.groupby(keys, as_index=False)
         .agg(
            NDC_542=("NDC_542",              "first"),
            NDC    =("NDC_532",              "first"),   # output as NDC for downstream compatibility
            NDC8   =("NDC8",                 "first"),
    ...
    # --- compute PriorScore per (NDC11, Year) ---
...
def attach_prior_score_to_tests(test_df: pd.DataFrame, events: pd.DataFrame) -> pd.DataFrame:
    """
    FEI-wise prior score attachment via merge_asof (per-FEI to avoid global sort requirement).
    ...
    e_by_fei = {
        fei: g.sort_values("EventDate", kind="mergesort").reset_index(drop=True)
    ...
    for fei, g in t.groupby("FEI", sort=False):
        ...
        merged = pd.merge_asof(
            gg, ev,
            left_on="TestDate", right_on="EventDate",
    ...
    merged_all["PriorScore_cat"] = pd.Categorical(
        merged_all["PriorScore"], categories=SCORE_ORDER, ordered=True
    )
    return merged_all
...
def report_price_volume_relationship(
    ...
    Checks the claim that the market focuses on price.
    """
    d = ndc_year_df.copy()
    ...
    def _one_report(tag: str, df_: pd.DataFrame) -> None:
        ...
        print(
            f"{tag}: Spearman: ρ={rho_s:+.3f}, p={p_s:.3e}, n={n} | {pearson_str}"
        )
...
def plot_obs1_country_bars(test_df: pd.DataFrame) -> None:
    """Observation 1: averages by country across all test records (bar plot)."""
    ...
    for ax, (col, title, fmt, ncol) in zip(axes, metrics):
        ...
        for xi, n in zip(x, nvals):
            ax.text(xi, y_n, f"n={int(n)}", ha="center", va="center",
...
def print_fig2_summary_stats(
    ...
    """
    Prints mean/median/IQR by PriorScore for price and volume,
    using the same inclusion rules as Figure 2.
    """
...
def plot_obs3_scatter_volume_price_vs_quality(test_df: pd.DataFrame) -> None:
    """
    Observation 3: scatter of volume and price vs quality metrics (DMF, NDMA, Difference Factor).
    Difference Factor NaN rows are excluded from that panel only.
    Spearman correlation computed on exactly the plotted sample.
    """
...
# Backs the claim that warning letters do not obviously change volume.
# ================================================================================
...
# -------------------- WARNING LETTER PLOTS --------------------
# v10_v4 already has ndc11 — use it directly, no format conversion needed.
for case in WARNING_LETTER_CASES:
    fei     = str(case["fei"])
    wl_date = pd.Timestamp(case["wl_date"])
    label   = case.get("label", fei)

    # Get all ndc11 values for this FEI from the Q&A data
    ndc11_list = (
        df.loc[df["FEI"].astype(str) == fei, "ndc11"]
        .dropna()
        .astype(str)
        .str.replace(r"\D", "", regex=True)
        .str.zfill(11)
        .where(lambda s: s.str.len() == 11)
        .dropna()
        .unique()
        .tolist()
    )
    ...
    # Filter monthly panel and aggregate by month
    ...
    series = (
        m.groupby("date", as_index=False)
         .agg(
            units   =("iqvia_extended_units", "sum"),
            n_rows  =("date",                 "size"),
    ...
    # Window: ±24 months around warning letter date
    ...
    plt.title(
        f"IQVIA extended units (sum over FEI-linked NDCs)\n"
        f"{label} | FEI={fei} | ndc11={len(ndc11_list)}"
    )
    plt.xlabel("Month")
    ...
    print(f"{label} | FEI={fei} | WL={wl_date.date()} | ndc11={len(ndc11_list)} | monthly_rows={len(m):,}")
