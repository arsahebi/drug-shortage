...
def _cgm_vcov(y, X, c1, c2, beta=None):
    ...
    beta: optional external coefficient vector (e.g. from MixedLM/RE).
          If None, uses OLS beta. When beta is from RE model, residuals
          are computed from RE fit so the sandwich correctly reflects
          RE + two-way clustered SE as described in the paper Methods.
    """
    n, k = X.shape
...
""")
    ...
    for metric, years in METRIC_YEARS.items():
        ...
        if metric == "Dissolution" or n_ndc < 3:
            ...
            # Step 1: Random Effects (MixedLM) — beta used as PRIMARY
            try:
                ...
                beta_re = np.array([mlm.params.get("Intercept", np.nan),
                                    mlm.params.get("IND", np.nan),
                                    mlm.params.get("CHN", np.nan)])
            except Exception as exc:
                print(f"  MixedLM error: {exc} -- falling back to OLS beta")

            # Step 2: CGM two-way clustered SE applied to RE beta (PRIMARY)
            if beta_re is not None and not np.any(np.isnan(beta_re)) and has_ndc and has_fei:
                try:
                    V2 = _cgm_vcov(y, X,
                                   sub[NDC_COL].values, sub[FEI_COL].values,
                                   beta=beta_re)
                    se2 = np.sqrt(np.diag(V2))
                    _coef_table(["const", "IND", "CHN"], beta_re, se2, dof,
                                header=f"★ RE beta + TWO-WAY clustered SE [{ylabel}] -- PRIMARY:")
                except Exception as exc:
                    print(f"  RE+CGM error: {exc}")
                    beta_re = None  # trigger OLS fallback below

            # OLS + two-way clustered SE — kept as comparison
            print(f"\n  COMPARISON — OLS beta + two-way clustered SE [{ylabel}]:")
            _ols_clustered(y, X, sub, ["IND", "CHN"], ylabel,
                           f"B_RE_2way_{metric}", output_dir)

            # If RE failed, mark OLS as primary
            if beta_re is None:
                print("  (NOTE: RE failed — OLS above is the effective primary)")
...
""")
    ...
    for outcome in FIG2_OUTCOMES:
        ...
        beta_re2 = None
        dof2 = max(n_obs - 3, 1)
        has_ndc2 = NDC_COL in sub.columns and sub[NDC_COL].nunique() >= 2
        has_fei2 = FEI_COL in sub.columns and sub[FEI_COL].nunique() >= 2
        ...
        if n_ndc >= 3:
            try:
                ...
                print(f"\n  RE model | ICC={icc:.4f}  "
                      f"Var(NDC)={var_re:.4f}  Var(resid)={var_res:.4f}")
                beta_re2 = np.array([mlm.params.get("Intercept", np.nan),
                                     mlm.params.get("VAI", np.nan),
                                     mlm.params.get("OAI", np.nan)])
            except Exception as exc:
                print(f"  MixedLM error: {exc} -- falling back to OLS beta")

        # Step 2: CGM two-way clustered SE applied to RE beta (PRIMARY)
        if beta_re2 is not None and not np.any(np.isnan(beta_re2)) and has_ndc2 and has_fei2:
            try:
                V2b = _cgm_vcov(y, X,
                                sub[NDC_COL].values, sub[FEI_COL].values,
                                beta=beta_re2)
                se2b = np.sqrt(np.diag(V2b))
                _coef_table(["const", "VAI", "OAI"], beta_re2, se2b, dof2,
                            header=f"★ RE beta + TWO-WAY clustered SE [{ylabel}] -- PRIMARY (ref=NAI):")
            except Exception as exc:
                print(f"  RE+CGM error: {exc}")
                beta_re2 = None

        # OLS + two-way clustered SE — kept as comparison
        print(f"\n  COMPARISON — OLS beta + two-way clustered SE [{ylabel}] (reference = NAI):")
