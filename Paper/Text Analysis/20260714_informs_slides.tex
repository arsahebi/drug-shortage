...
\begin{itemize}
  ...
  \item<1-> \textbf{Same inspection, two different domains} -- Data Integrity vs.\ Lab Controls, each carrying different risk implications
  \item<2-> Domain, severity, and patient risk are embedded in the prose; there are no structured fields or shared schema -- extracting them requires reading the text
  \item<3-> Observations share a standard CFR citation header, but the body is inspector-written and facility-specific. Pairwise Jaccard similarity across 1,067 observations averages 0.10 -- nearly all vocabulary is unique to each observation.
\end{itemize}
}
\end{frame}
