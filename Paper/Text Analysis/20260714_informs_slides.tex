...
\begin{frame}{483 Severity Signal: The Quality Problem Predates the FDA Visit}
\begin{columns}[T]
\column{0.56\textwidth}
\textbf{Spearman $\rho$ one quarter before inspection (Q-1) and at inspection (Q0), manufacturer-specific AEs:}

\vspace{4pt}
{\small
\begin{tabular}{lcclc}
\toprule
Feature & $\rho_{\text{Q-1}}$ & & $\rho_{\text{Q0}}$ & \\
\midrule
\texttt{severity\_critmajor}    & \tech{+0.18} & *  & \tech{+0.23} & ** \\
\texttt{data\_integrity\_flag}  & +0.13 &    & +0.14 &    \\
\texttt{n\_labcontrols\_obs}    & +0.15 &    & +0.10 &    \\
\texttt{vc\_labcontrols\_share} & +0.14 &    & +0.07 &    \\
\midrule
\texttt{cultural\_root\_cause}  & $-$0.04 &    & +0.03 &    \\
\bottomrule
\multicolumn{5}{l}{\scriptsize * $p{<}.05$\quad ** $p{<}.01$ \quad Full heatmap: backup slide}
\end{tabular}
}
...
\begin{itemize}
  ...
  \item \textbf{Severity} is the clearest signal -- significant both before (Q-1) and at the time of inspection (Q0)
  \item Facilities with more severe 483 findings were already generating more AEs before FDA arrived -- the problem was present before the visit
  \item Lab Controls and DI features are positive but not statistically significant at Q0
\end{itemize}
