...
% --- Correlation table ---
\begin{frame}{Backup: 483 Text Signals -- Two Snapshots, Two Different Stories}
\begin{columns}[T]
...
\bottomrule
\multicolumn{5}{l}{\scriptsize * $p{<}.05$\quad ** $p{<}.01$\quad *** $p{<}.001$}
\end{tabular}
}
...
% --- Model setup ---
\begin{frame}{Backup: Prediction Setup -- 483 Text vs.\ Inspection Outcome Flag}
\begin{columns}[T]
\column{0.50\textwidth}
...
\begin{itemize}
  \setlength{\itemsep}{5pt}
  \item \textbf{Unit:} one inspection event (176 with ANDA-matched AEs)
  ...
  \item \textbf{CV:} FEI-grouped 5-fold logistic regression
  \item \textbf{Baseline:} AUC\,=\,0.5 (50/50 by construction)
\end{itemize}
}

\column{0.47\textwidth}
\textbf{Why focus on VAI-only?}

\vspace{6pt}
\begin{itemize}
  \setlength{\itemsep}{8pt}
  \item \highlight{55 facilities} never received an OAI -- every inspection ended VAI or NAI
  \item Within this group the outcome label never varies; text is the \emph{only} available signal to differentiate facilities
  \item This is the hardest and most policy-relevant test: can text find the high-risk facilities that FDA's classification missed?
\end{itemize}
