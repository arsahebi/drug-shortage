...
\midrule
\highlight{D} & \highlight{Text only, VAI-only facilities} & \highlight{0.586} & $p{=}0.017$ \\
...
\column{0.47\textwidth}
{\small
\textbf{Full sample (A--C):} Text alone is near random (0.503). The inspection outcome flag alone reaches only 0.539 -- even with the label, discrimination is weak.

\vspace{5pt}
\textbf{VAI-only (D):} Among the 73 facilities FDA never penalized, text reaches \highlight{AUC\,=\,0.59} ($p{=}0.017$). This is not a strong classifier -- but it is detectable signal in a subgroup where no other differentiator exists.

\vspace{5pt}
\textbf{OAI-ever (E):} Text adds little where enforcement has already acted -- consistent with OAI correcting the underlying problems.

\vspace{5pt}
\textbf{Takeaway:} 483 text carries detectable signal precisely where FDA's classification provides none -- using text features alone, with no volume or market controls.

\vspace{5pt}
{\footnotesize \textit{A complete prediction model would also incorporate facility size, market concentration, and pricing signals. The goal here is to isolate the unique contribution of inspection text.}}
}
\end{columns}
\end{frame}
