...
\begin{itemize}
  ...
  \item \textbf{90\%} of U.S. prescriptions are generic drugs -- scale makes quality oversight critical
  \item FDA inspects each manufacturing facility \textbf{every 1--3 years} on average
  \item Between visits, FDA receives quality signals through \textbf{recalls, adverse event reports, and drug complaints} -- but real-time insight into manufacturing conditions is limited
  \item When problems go undetected, they can escalate: \textbf{FDA reports that two-thirds of drug shortages stem from manufacturing quality failures}
  \item \textbf{Catching quality signals early} -- before they surface as patient harm or supply disruption -- is the central challenge
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item Issued only at \textbf{violative inspections} -- when GMP deficiencies are found
  \item Most inspections \textbf{do not} result in a 483
  \item \textbf{3--15 observations} per form, each with inspector-written detail specific to that facility
  \item Covers contamination, data integrity, lab controls, quality systems
  \item \textbf{Not public} -- obtained through FOIA; our data acquired via Redica
  \item \textbf{Rich, facility-specific text} that carries information beyond the inspection outcome alone
\end{itemize}
...
\begin{itemize}
  \item Most VAI facilities carry the same outcome label -- no structured public signal differentiates them
  \item 483 text must be FOIA'd and takes time to obtain -- but once available, it captures deficiency detail that predates recalls and adverse events
  \item The detailed content of 483 observations has not been used in research for quality prediction
\end{itemize}

\vspace{6pt}
\begin{alertblock}{Research question}
\small
Do 483 text signals predict serious adverse events -- and can they differentiate facilities that carry the same inspection outcome label?
\end{alertblock}
\end{columns}
\end{frame}
...
\begin{itemize}
  \item 129 FEIs (Facility Establishment Identifiers -- unique FDA facility codes) via DailyMed NDC$\to$FEI crosswalk
...
Source & Role \\
\midrule
\textbf{Redica} & 483 observation text; inspection outcomes (NAI/VAI/OAI) \\[4pt]
\textbf{FAERS} & FDA adverse event reports -- our outcome variable (serious AEs per facility per quarter) \\[4pt]
\textbf{DailyMed} & FDA's official drug label repository -- we extract the NDC$\to$ANDA$\to$FEI linkage from product labels \\
\bottomrule
\end{tabular}
}

\vspace{6pt}
\textbf{Panel:} one row per inspection, $\pm$4 quarters. 246 rows total.
\end{columns}
\end{frame}
...
\begin{columns}[T]
\column{0.50\textwidth}
\textbf{Outcome: serious adverse event reports}
\begin{itemize}
  \setlength{\itemsep}{5pt}
  \item Serious outcomes only: death, hospitalization, life-threatening, disability, congenital anomaly
  \item ``No outcome reported'' excluded (14.6\% of reports)
  \item Each report carries an ANDA number -- linked through the DailyMed crosswalk to assign the AE to the producing facility
  \item 144,286 serious reports $\to$ \textbf{68,500} matched and aggregated to quarterly counts per FEI
  \item When one manufacturer operates multiple sites under one ANDA, facility attribution is approximate
\end{itemize}

\column{0.47\textwidth}
\textbf{Sample coverage}\\[6pt]
\begin{tabular}{ll}
\toprule
Metric & Value \\
\midrule
