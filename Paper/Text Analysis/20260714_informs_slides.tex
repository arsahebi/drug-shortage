...
\begin{itemize}
  ...
  \item FDA inspects each manufacturing facility roughly every 1--3 years
  \item FDA monitors quality between visits through recalls, adverse event reports, and drug complaints -- but real-time visibility into ongoing manufacturing conditions is limited
  \item Quality problems that go undetected between visits can escalate to recalls and supply disruptions -- FDA reports that \textbf{two-thirds of drug shortages} stem from manufacturing quality failures
\end{itemize}
...
\begin{tikzpicture}[x=0.80cm, y=0.85cm]
  ...
  \node[font=\scriptsize, ncsuRed, align=center] at (1.5,0.55) {$\approx$1--3 yr gap\\limited visibility};
  \node[font=\scriptsize, ncsuRed, align=center] at (3.5,0.55) {$\approx$1--3 yr gap\\limited visibility};
...
\begin{frame}{The Form 483: Detailed, FOIA'd, and Underused}
\begin{columns}[T]
\column{0.50\textwidth}
\textbf{What is a Form 483?}
\begin{itemize}
  \item Issued only at the close of \emph{violative} inspections -- when GMP (Good Manufacturing Practice) deficiencies are found. Most inspections do not result in a 483.
  \item Lists each GMP deficiency observed (typically 3--15 per visit), with inspector-written detail specific to that facility
  \item Topics include contamination, data integrity, lab controls, and quality systems
  \item Not publicly available -- must be FOIA'd. Our data was obtained through Redica, which acquires 483s systematically via FOIA.
\end{itemize}
...
\vspace{8pt}
{\small Plain regulatory prose, specific to each inspection. No structured database exists -- but the text has never been systematically used in research to predict future quality failures.}
\end{columns}
\end{frame}
