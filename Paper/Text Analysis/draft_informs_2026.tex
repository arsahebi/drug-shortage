...
\begin{itemize}[noitemsep]
  ...
  \item \textit{Cross-inspection repeat detection}: the current extraction
        pipeline identifies repeated observations within a single 483 document
        (intra-inspection repeats) but does not cross-reference violations
        across multiple inspections of the same facility over time.
        Manual review of FEI 3011905047 (sterile injectable manufacturer)
        revealed identical sterility and aseptic-technique violations in both
        a 2019 and a 2024 FDA inspection, yet the pipeline scores
        \texttt{repeat\_llm\_share = 0}. Cross-inspection repeat detection
        — which would substantially raise the risk signal for chronic violators
        — is planned as a next-generation enhancement.
