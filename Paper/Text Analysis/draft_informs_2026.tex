...
\begin{enumerate}[noitemsep]
  ...
  \item A validated LLM extraction pipeline (GPT-5-mini with strict JSON schema
  ...
  \item A time-aware facility-level feature panel: cumulative text-signal
        ...
        years apart---encoding behavioral taxonomy consistent with Agency
        Theory and Remediation Economics.
  \item Empirical validation showing that LLM-graded severity tracks real
        inspection outcomes (Critical+Major share separates OAI from VAI
        inspections by 20 percentage points), and that text-derived features
        carry information absent from structured databases.
        \todo{Re-run shortage model with revised raw text features and update
